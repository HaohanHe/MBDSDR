/*
 * ai-sdr Mini Firmware v0.6 (SI4732 I2S直连 + WebOTA + MCP OTA)
 *   v0.6: 引脚对齐硬件 v0.7.1 — IMU_INT=IO12, MAG_INT=IO13,
 *         SI4732_RST 独占 IO11 (旧版与 IMU_INT 复用已拆开), 修注释
...
#define PIN_SI4732_RST    11   // IO11 -> SI4732 RST (v0.7.1起独占, 不再与IMU_INT复用)
#define PIN_IMU_INT       12   // IO12 -> BMI260 INT1 (数据就绪中断, 当前I2C轮询, 脚已接好)
#define PIN_MAG_INT       13   // IO13 -> TMAG5273 INT (数据就绪中断, 当前I2C轮询, 脚已接好)
...
void mcp_handleCall(uint8_t client, uint32_t id, const String& method, const String& params) {
  if (method == "tune_fm") {
    ...
    String tools = "[";
    tools += "{\"name\":\"tune_fm\",\"desc\":\"调谐FM广播, freq_mhz单位MHz\",\"params\":{\"freq_mhz\":\"float 64-108\"}},";
    tools += "{\"name\":\"tune_am\",\"desc\":\"调谐AM/MW广播, freq_khz单位kHz\",\"params\":{\"freq_khz\":\"int 531-1710\"}},";
    tools += "{\"name\":\"set_volume\",\"desc\":\"设置音量 0-63\",\"params\":{\"volume\":\"int 0-63\"}},";
    tools += "{\"name\":\"get_status\",\"desc\":\"获取当前SDR状态(模式/频率/RSSI/SNR)\",\"params\":{}},";
    tools += "{\"name\":\"get_gps\",\"desc\":\"获取GPS/北斗定位(经纬度/高度/卫星数/HDOP)\",\"params\":{}},";
    tools += "{\"name\":\"get_imu\",\"desc\":\"获取9轴姿态(加速度/陀螺仪/磁力计/温度)\",\"params\":{}},";
    tools += "{\"name\":\"start_record\",\"desc\":\"开始I2S基带录音(WAV头通过WebSocket推送,电脑端存盘)\",\"params\":{}},";
    tools += "{\"name\":\"stop_record\",\"desc\":\"停止录音,返回采样数\",\"params\":{}},";
    tools += "{\"name\":\"get_version\",\"desc\":\"获取固件版本/硬件/MCU/SDR芯片信息\",\"params\":{}},";
    tools += "{\"name\":\"check_update\",\"desc\":\"检查固件更新(返回当前版本/GitHub release地址/OTA地址)\",\"params\":{}},";
...
void setup() {
  ...
  delay(1000);
  Serial.println("\n========================================");
  Serial.println("  MBDSDR Mini Firmware v0.6 (hw v0.7.1)");
  ...
  // IMU/MAG 中断脚 (v0.7.1: BMI260 INT1=IO12, TMAG INT=IO13)
  pinMode(PIN_IMU_INT, INPUT_PULLUP);
  pinMode(PIN_MAG_INT, INPUT_PULLUP);
  Serial.println("[INT] IMU_INT=IO12, MAG_INT=IO13 (input pull-up, poll mode)");
